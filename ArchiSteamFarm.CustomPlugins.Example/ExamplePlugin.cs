...
namespace ArchiSteamFarm.CustomPlugins.Example {
	...
	internal sealed class ExamplePlugin : IASF, IBot, IBotCommand, IBotConnection, IBotMessage, IBotModules {
		...
		// Please note that this function can have direct dependencies only on structures that were initialized by the constructor, as it's possible to be called before OnLoaded() takes place
		public string GetName() => nameof(ExamplePlugin);
		...
		// Please note that this function can have direct dependencies only on structures that were initialized by the constructor, as it's possible to be called before OnLoaded() takes place
		public Version GetVersion() => typeof(ExamplePlugin).Assembly.GetName().Version;
