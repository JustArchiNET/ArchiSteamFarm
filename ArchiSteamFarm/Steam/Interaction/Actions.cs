...
public sealed class Actions : IAsyncDisposable, IDisposable {
	...
	[PublicAPI]
	public async Task<(HashSet<Asset>? Result, string Message)> GetInventory(uint appID = Asset.SteamAppID, ulong contextID = Asset.SteamCommunityContextID, string language = "english", Func<Asset, bool>? filterFunction = null) {
		ArgumentOutOfRangeException.ThrowIfZero(appID);
